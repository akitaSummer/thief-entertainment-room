...
class Game {
  ...
  pokerList: Poker[];
  playerList: Player[];
  dizhuPokerList: Poker[];
  oldPokerList: Poker[];
  deskPokerObj: Poker | null;
  currentPlayer: Player | null;
  currentJiaoFenPlayer: Player | null;
  jiaoFenCount: number;
  dizhu: Player | null;
  MaxSecond: number;
  second: number;
  ...
  someOneJiaoFen() {
    ...
    if (this.jiaoFenCount === 3) {
      ...
      this.currentJiaoFenPlayer = this.currentJiaoFenPlayer?.next || null;
    }
  }
  ...
  next() {
    this.resetTime();
    this.currentPlayer = this.currentPlayer?.next || null;
    if (this.currentPlayer?.isRobot) {
      this.currentPlayer.playByAI();
    }
  }
